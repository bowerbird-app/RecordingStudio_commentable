...
class CommentsPagesFlowTest < Minitest::Test
  ...
  def test_engine_home_lists_comments_with_back_navigation
    ...
    assert_includes controller_source, "@comment_entries = visible_comment_entries"
    assert_includes controller_source, "return main_app.scenarios_path if main_app.respond_to?(:scenarios_path)"
    ...
    assert_includes view_source, 'render "recording_studio_commentable/comments/comment"'
  end
  ...
  def test_new_comment_page_and_comment_actions_preserve_return_to_flow
    ...
    assert_includes comment_partial_source,
                    "edit_recording_comment_path(parent_recording, comment_recording, return_to: params[:return_to])"
    assert_includes comment_partial_source,
                    "recording_comment_path(parent_recording, comment_recording, return_to: params[:return_to])"
  end
