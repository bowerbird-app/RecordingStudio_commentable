...
module RecordingStudioCommentable
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      desc "Installs RecordingStudioCommentable engine into your application"
      ...
      def mount_engine
        route %(mount RecordingStudioCommentable::Engine, at: "#{options[:mount_path]}")
      end

      def copy_initializer
        template "recording_studio_commentable_initializer.rb",
                 "config/initializers/recording_studio_commentable.rb"
      end
      ...
      def add_tailwind_source
        ...
        if missing_lines.empty?
          say "Tailwind already configured to include RecordingStudioCommentable and FlatPack sources.", :green
          return
        end
      ...
      def inject_tailwind_sources(tailwind_css_path, missing_lines)
        ...
        say "Added RecordingStudioCommentable and FlatPack sources to Tailwind CSS configuration.", :green
        say "Run 'bin/rails tailwindcss:build' to rebuild your CSS.", :green
      end

      def formatted_tailwind_source_block(missing_lines)
        [
          "\n/* Include RecordingStudioCommentable engine views for Tailwind CSS */",
